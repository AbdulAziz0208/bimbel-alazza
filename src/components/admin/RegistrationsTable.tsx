        ...
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg overflow-hidden flex flex-col max-h-[90vh]">
            ...
            <form onSubmit={handleSaveEdit} className="p-6 space-y-4 overflow-y-auto">
              ...
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1">Target/Harapan (Learning Goal)</label>
                <textarea
                  className="w-full px-4 py-2 border border-slate-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                  rows={2}
                  value={editingRow.learning_goal || ''}
                  onChange={e => setEditingRow({...editingRow, learning_goal: e.target.value})}
                />
              </div>

              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1">Jumlah Pertemuan</label>
                <input
                  type="text"
                  className="w-full px-4 py-2 border border-slate-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                  value={editingRow.meeting_frequency || ''}
                  onChange={e => setEditingRow({...editingRow, meeting_frequency: e.target.value})}
                />
              </div>
